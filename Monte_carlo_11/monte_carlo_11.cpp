...
int main()
{
    ...
    std::ofstream out("C:\\Users\\kacpe\\OneDrive\\Pulpit\\C_plus\\Monte_Carlo\\Monte_carlo_11\\monte_carlo_11.txt");
    for (int j = ob.ny; j >= 0; j--) {  // od góry do dołu, zgodnie z układem współrzędnych
        for (int i = 0; i <= ob.nx; i++) {
            out << ob.absorption[i][j] << " ";
        }
        out << "\n";
    }
    out.close();
